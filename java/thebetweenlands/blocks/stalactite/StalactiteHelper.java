...
public class StalactiteHelper
{
    ...
    public Point2D getMidway(double t)
    {
        return new Point2D(bX + (tX - bX) * t, bZ + (tZ - bZ) * t);
    }

    protected static int getValFor(int x, int y, int z, int size)
    {
        int seed = 0;
    ...
    public static int moduloI(int input, int n)
    {
        int r = input % n;
        if (r < 0) r += n;
        return r;
    ...
    public static float randRange(Random rand, float min, float max)
    {
