...
public class WaterSwampReedRenderer implements IWaterRenderer {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID,
		...
		renderer.drawCrossedSquares(((BlockSwampReedUW)block).iconSwampReed, -0.5, -0.5, -0.5, 1.0f);
		tessellator.draw();
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z,
		...
		Tessellator.instance.setColorOpaque(255, 255, 255);
		renderer.drawCrossedSquares(((BlockSwampReedUW)block).iconSwampReed, x, y, z, 1.0f);
		Block blockAbove = world.getBlock(x, y+1, z);
		if(blockAbove == BLBlockRegistry.swampWater || blockAbove == Blocks.air) {
			renderer.drawCrossedSquares(((BlockSwampReedUW)block).iconSwampReedTop, x, y+1, z, 1.0f);
		}
		return true;
	}
}
