...
public class LifeCrystalRenderer implements IWaterRenderer {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID,
		...
		renderBlock(block, 0, 0, 0, true, 1, true, 0, mc.theWorld.getLightBrightnessForSkyBlocks(
				(int)(mc.thePlayer.posX), (int)(mc.thePlayer.posY), (int)(mc.thePlayer.posZ), 0), BLBlockRegistry.limestone.getIcon(0, 0));
		tessellator.draw();
	...
	{
		LifeCrystalData info = LifeCrystalData.getData(blockAccess, x, y, z);
		if(blockAccess.getBlockMetadata(x, y, z) == 0) {
			return renderBlock(block, info.posX, info.posY, info.posZ, info.noBottom, info.distDown, info.noTop, info.distUp, block.getMixedBrightnessForBlock(blockAccess, x, y, z), BLBlockRegistry.pitstone.getIcon(0, 0));
		}
		return renderBlock(block, info.posX, info.posY, info.posZ, info.noBottom, info.distDown, info.noTop, info.distUp, block.getMixedBrightnessForBlock(blockAccess, x, y, z), BLBlockRegistry.lifeCrystalOre.iconBackground)
				&& renderBlock(block, info.posX, info.posY, info.posZ, info.noBottom, info.distDown, info.noTop, info.distUp, 240, BLBlockRegistry.lifeCrystalOre.iconGlowing);
	}

	public static boolean renderBlock(Block block, int _x, int _y, int _z, boolean noBottom, int distDown, boolean noTop, int distUp, int brightness, IIcon icon)
	{
	...
	public IIcon getIcon() {
		return BLBlockRegistry.pitstone.getIcon(0, 0);
	}
}
