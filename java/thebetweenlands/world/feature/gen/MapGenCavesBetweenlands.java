...
public class MapGenCavesBetweenlands extends MapGenBase {
    ...
    private boolean isExceptionBiome(BiomeGenBase biome)
    {
        ...
        if (biome instanceof BiomeGenBaseBetweenlands) return true;
        return false;
    }
    ...
    protected void digBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
    {
        ...
        Block underLayerBlock = null;
        if(biome instanceof BiomeGenBaseBetweenlands) {
        	baseBlock = ((BiomeGenBaseBetweenlands)biome).getBaseBlock();
        	underLayerBlock = ((BiomeGenBaseBetweenlands)biome).getUnderLayerBlock();
        }

        if (block == baseBlock || block == underLayerBlock || block == filler || block == top)
        {
            if (y < 10)
            {
