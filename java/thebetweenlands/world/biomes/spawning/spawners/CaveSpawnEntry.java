 ...
 * Spawning weight on the surface is baseWeight / 6. Spawning weight
...
public class CaveSpawnEntry extends BLSpawnEntry {
	...
	protected void update(World world, int x, int y, int z) {
		...
		short spawnWeight = (short) (this.baseWeight / 6);
		if(y < surfaceHeight) {
			double percentage = 1.0D - ((double)(surfaceHeight - y) / (double)surfaceHeight);
			spawnWeight = (short) MathHelper.ceiling_double_int(this.baseWeight / (5.0D * percentage + 1.0D));
		}
