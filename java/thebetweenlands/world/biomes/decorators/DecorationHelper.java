...
public class DecorationHelper {
	...
	private final static WorldGenMudStructures GEN_MUD_STRUCTURES = new WorldGenMudStructures();
	...
	public void generateSmallWeedwoodTree(int attempts) {
		if (canShortThingsGenerateHere()) {
			for (int i = 0; i < attempts; i++) {
				int x = this.x + rand.nextInt(12) + 10;
				int y = this.y - 8 + this.rand.nextInt(16);
	...
	public void generateMudStructures(int attempt) {
		for (int i = 0; i < attempt; i++) {
			...
			int y = this.y - 8 + this.rand.nextInt(16);
			int z = this.z + this.offsetXZ();
			GEN_MUD_STRUCTURES.generate(world, rand, x, y, z);
		}
	}
