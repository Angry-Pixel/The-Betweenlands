...
public class LightningArcParticle extends Particle {
	...
	protected LightningArcParticle(ClientLevel level, double xIn, double yIn, double zIn, double mx, double my, double mz, Vec3 target) {
	...
	public LightningArcParticle setBaseSize(float size) {
		this.baseSize = size;
		return this;
	}
	
	public LightningArcParticle setSubdivs(int baseSubdivs, int branchSubdivs) {
		this.baseSubdivs = baseSubdivs;
		this.branchSubdivs = branchSubdivs;
		return this;
	}
	
	public LightningArcParticle setOffsets(float baseOffsets, float branchOffsets) {
		this.baseOffsets = baseOffsets;
		this.branchOffsets = branchOffsets;
		return this;
	}
	
	public LightningArcParticle setSplits(int splits) {
		this.splits = splits;
		return this;
	}

	public LightningArcParticle setSplitSpeed(float minSplitSpeed, float maxSplitSpeed) {
		this.minSplitSpeed = minSplitSpeed;
		this.maxSplitSpeed = maxSplitSpeed;
		return this;
	}
	
	public LightningArcParticle setLengthDecay(float multiplier) {
		this.lengthDecay = multiplier;
		return this;
	}
	
	public LightningArcParticle setSizeDecay(float multiplier) {
		this.sizeDecay = multiplier;
		return this;
	}
	
	public LightningArcParticle setLighting(boolean light) {
		this.lighting = light;
		return this;
	}
	...
	public void render(VertexConsumer buffer, Camera camera, float partialTicks) {	
		...
		float scale = ((Mth.sin((this.age + partialTicks) * 0.8f) + 1) * 0.5f * 0.5f + 0.5f) * this.baseSize * (1 - (this.age - 1 + partialTicks) / this.lifetime);
		...
		if(this.lighting && ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			if(distFromCam < 40) {
	            ...
	            ShaderHelper.INSTANCE.getWorldShader().addLight(new LightSource(this.target.x, this.target.y, this.target.z,
	                    1.0f + 5.0f * this.baseSize, this.rCol * strength, this.gCol * strength, this.bCol * strength));
			}
        }
	}
	...
	@Override
	 public void tick() {
		...
		if(this.arcs.isEmpty()) {
			this.addArc(this.arcs, new Arc(null, 0, Vec3.ZERO, target.subtract(this.x, this.y, this.z), this.splits, 0, 0, 1.0f), this.baseOffsets, this.baseSubdivs);
		}
			...
			for(Arc arc : this.arcs) {
				if(arc.splits > 0) {
					...
					for(int i = 0; i < numSplits; i++) {
						...
						Vec3 dir = arc.dir.add((this.random.nextFloat() - 0.5f) * len * 0.5f, (this.random.nextFloat() - 0.5f) * len * 0.5f, (this.random.nextFloat() - 0.5f) * len * 0.5f).normalize().scale(len * arc.subdivs * this.lengthDecay);
	...
	public static final class Factory extends ParticleFactory<Factory, SimpleParticleType> {
		...
		@Override
		public LightningArcParticle createParticle(SimpleParticleType type, ImmutableParticleArgs args) {
			return new LightningArcParticle(args.level, args.x, args.y, args.z, args.motionX, args.motionY, args.motionZ, args.data.getObject(Vec3.class, 0));
		}
