...
public class SpiritButterflyParticle extends BugParticle {
	public SpiritButterflyParticle(ClientLevel level, double x, double y, double z, double mx, double my, double mz, int maxAge, float speed, float jitter, float scale) {
		super(level, x, y, z, mx, my, mz, maxAge, speed, jitter, scale, false);
	}
	...
	public static final class Factory extends ParticleFactory<Factory, SimpleParticleType> {
		...
		public BugParticle createParticle(SimpleParticleType type, ImmutableParticleArgs args) {
			var particle = new SpiritButterflyParticle(args.level, args.x, args.y, args.z, args.motionX, args.motionY, args.motionZ, args.data.getInt(0), args.data.getFloat(1), args.data.getFloat(2), args.scale);
