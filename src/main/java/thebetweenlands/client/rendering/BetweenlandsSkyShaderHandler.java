...
public class BetweenlandsSkyShaderHandler {
	...
	public void setup(ResourceManager resourcemanager) {
		...
		LongSupplier starttimenano = new LongSupplier() {
			public long getAsLong() {
				return 0;
			}
		};
		IntSupplier starttime = new IntSupplier() {
			public int getAsInt() {
				return 0;
			}
		};
		...
		// Setup atlas objects
		NoiseUV.set();
		SkyUV.forEach((AtlasObject object) -> {
			object.set();
		});
		EffectUV.forEach((AtlasObject object) -> {
			object.set();
		});
		...
		// Sky textures
		SkyUV.forEach((AtlasObject object) -> {
			Input.put(Float.floatToRawIntBits(object.u0));
