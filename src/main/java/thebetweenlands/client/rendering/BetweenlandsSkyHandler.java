...
public class BetweenlandsSkyHandler implements ISkyRenderHandler {
	...
	public void render(int ticks, float partialTick, PoseStack poseStack, ClientLevel level, Minecraft minecraft) {
		...
		levelrenderer.renderSky(poseStack, oldmatrix, partialTick, camera, false, () -> {FogRenderer.setupFog(camera, FogRenderer.FogMode.FOG_TERRAIN, Math.max(minecraft.gameRenderer.getRenderDistance(), 32.0F), false, partialTick);});
