...
public class RenderFortressBossTeleporter extends Render<EntityPrimordialMalevolenceTeleporter> {
	...
	public void doRender(EntityPrimordialMalevolenceTeleporter entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		GlStateManager.translatef(0, -0.25F, 0.1F);
		GlStateManager.translatef(MODEL.eye.rotationPointX * 0.065F / 2.0F, MODEL.eye.rotationPointY * 0.065F / 2.0F, MODEL.eye.rotationPointZ * 0.065F / 2.0F);
		if(entity.isLookingAtPlayer || entity.getTarget() != null) {
			...
			GlStateManager.rotatef(88, 1, 0, 0);
		}
		GlStateManager.translatef(-MODEL.eye.rotationPointX * 0.065F / 2.0F, -MODEL.eye.rotationPointY * 0.065F / 2.0F + 0.3F, -MODEL.eye.rotationPointZ * 0.065F / 2.0F);
		...
		if(entity.getTarget() == null || entity.getTarget() != Minecraft.getInstance().getRenderViewEntity()) {
			...
			GlStateManager.translatef(0, scale / 2.0F, 0);
			GlStateManager.scalef(0.8F + scale, 0.8F + scale, 0.8F);
		}
