...
public final class ModelSwampWater implements IModelCustomData {
	public static final ModelSwampWater WATER = new ModelSwampWater(FluidRegistry.WATER);

	private final Fluid fluid;
	private final ModelResourceLocation model;

	public ModelSwampWater(Fluid fluid) {
		this(fluid, null);
	}

	public ModelSwampWater(Fluid fluid, ModelResourceLocation model) {
		this.fluid = fluid;
		this.model = model;
	}
	...
	public IBakedModel bake(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
		...
		return new ModelBakedSwampWater(this.model, state.apply(Optional.<IModelPart>absent()), map, format, fluid.getColor(), bakedTextureGetter.apply(fluid.getStill()), bakedTextureGetter.apply(fluid.getFlowing()), fluid.isGaseous(), Optional.<IExtendedBlockState>absent());
	}
	...
	private static final class ModelBakedSwampWater implements IPerspectiveAwareModel, IBakedModelDependant {
		private static final int x[] = { 0, 0, 1, 1 };
		private static final int z[] = { 0, 1, 1, 0 };
		private static final float eps = 1e-3f;

		private final LoadingCache<Long, ModelBakedSwampWater> fluidModelCache = CacheBuilder.newBuilder().maximumSize(200).build(new CacheLoader<Long, ModelBakedSwampWater>() {
			public ModelBakedSwampWater load(Long key) throws Exception {
				...
				return new ModelBakedSwampWater(modelLocation, transformation, transforms, format, color, still, flowing, gas, statePresent, cornerRound, flowRound);
			}
		});
		...
		public ModelBakedSwampWater(@Nullable ModelResourceLocation model, Optional<TRSRTransformation> transformation, ImmutableMap<TransformType, TRSRTransformation> transforms, VertexFormat format, int color, TextureAtlasSprite still, TextureAtlasSprite flowing, boolean gas, Optional<IExtendedBlockState> stateOption) {
			this(model, transformation, transforms, format, color, still, flowing, gas, stateOption.isPresent(), getCorners(stateOption), getFlow(stateOption));
		}
		...
		public ModelBakedSwampWater(@Nullable ModelResourceLocation model, Optional<TRSRTransformation> transformation, ImmutableMap<TransformType, TRSRTransformation> transforms, VertexFormat format, int color, TextureAtlasSprite still, TextureAtlasSprite flowing, boolean gas, boolean statePresent, int[] cornerRound, int flowRound) {
		...
		public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand) {
			ModelBakedSwampWater model = this;
			if(state instanceof IExtendedBlockState) {
				...
				key <<= 1;
				key |= 1;
				model = fluidModelCache.getUnchecked(key);
			}
		...
		public Collection<ModelResourceLocation> getDependencies(ResourceLocation modelLocation) {
			return this.modelLocation != null ? ImmutableSet.of(this.modelLocation) : Collections.emptySet();
		}

		@Override
		public void setDependencies(ResourceLocation modelLocation, Map<ModelResourceLocation, IBakedModel> dependencies) {
			this.model = dependencies.get(this.modelLocation);
		}
	}

	@Override
	public ModelSwampWater process(ImmutableMap<String, String> customData) {
		...
		return new ModelSwampWater(FluidRegistry.getFluid(fluid));
	}
}