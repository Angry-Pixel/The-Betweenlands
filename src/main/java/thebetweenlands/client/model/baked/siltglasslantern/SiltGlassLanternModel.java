...
public class SiltGlassLanternModel implements IDynamicBakedModel {
    ...
    public SiltGlassLanternModel(TextureAtlasSprite texture, TextureAtlasSprite ropeSprite, ItemTransforms transforms, Transformation identity) {
		...
		QuadBuilder ropeBuilder = new QuadBuilder(DefaultVertexFormat.BLOCK)
			...
			.setTintIndex(-1);
		SiltGlassLanternGeometry.buildRope(ropeBuilder);
		...
		for (int i = 0; i < 8; i++) {
			...
			QuadBuilder builder = new QuadBuilder(DefaultVertexFormat.BLOCK)
				...
				.setTintIndex(-1);
			SiltGlassLanternGeometry.build(builder);
			this.quadss[i] = builder.build().nonCulledQuads;
			SiltGlassLanternGeometry.buildInsides(builder);
			this.glassQuadss[i] = builder.build().nonCulledQuads;
		}
	}
	...
	public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, RandomSource rand, ModelData extraData, @Nullable RenderType renderType) {
		...
		if (state == null) return new CompositeList<>(this.quadss[0], this.glassQuadss[0]);
		int rotation = state.getValue(BLLanternBlock.ROTATION);
		boolean hanging = state.getValue(LanternBlock.HANGING);

		if (renderType == RenderType.translucent()) return this.glassQuadss[rotation];
        if (renderType == RenderType.cutout()) return hanging ? new CompositeList<>(this.quadss[rotation], this.lanternRope) : this.quadss[rotation];

        return new CompositeList<>(new CompositeList<>(this.quadss[rotation], this.lanternRope), this.glassQuadss[0]);
	}
