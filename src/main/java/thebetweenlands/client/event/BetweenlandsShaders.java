...
public class BetweenlandsShaders {
	...
	static void registerShaders(RegisterShadersEvent event) {
		try {
			event.registerShader(new ShaderInstance(event.getResourceProvider(), TheBetweenlands.prefix("primordial_shield/primordial_shield"), DefaultVertexFormat.POSITION_TEX_COLOR), instance -> {
				PRIMORDIAL_SHIELD = instance;
			});
			event.registerShader(new ShaderInstance(event.getResourceProvider(), TheBetweenlands.prefix("particle/particle_no_alpha"), DefaultVertexFormat.PARTICLE), instance -> {
				PARTICLE_NO_ALPHA_CHECK = instance;
			});
		} catch (IOException e) {
