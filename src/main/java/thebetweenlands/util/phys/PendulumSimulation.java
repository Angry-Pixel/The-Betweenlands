...
public final class PendulumSimulation implements Simulation {
	...
	public void move(final float x, final float y) {
		if (x != 0.0F || y != 0.0F) {
			this.moveAngle = -(float) Mth.atan2(x, -y);
			this.moveForce = Mth.sqrt(x * x + y * y);
		} else {
			this.moveAngle = 0.0F;
			this.moveForce = 0.0F;
		}
	...
	public void evaluate(final float[] state, final float[] rate) {
		...
		final float alpha = -(this.moveForce / this.length) * Mth.sin(theta + this.moveAngle) -
			(G / this.length) * Mth.sin(theta) -
