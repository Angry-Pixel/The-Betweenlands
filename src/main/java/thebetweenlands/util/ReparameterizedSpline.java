...
public class ReparameterizedSpline implements ISpline {
	...
	public ReparameterizedSpline init(int subdivs, int segmentSubdivs) {
		...
		Vec3[] arcLengths = new Vec3[subdivs + 2];
		double delta = this.length / (subdivs - 1);
		for(int i = 0; i < subdivs; i++) {
			arcLengths[i+1] = new Vec3(this.getCurveParameter(delta * i), 0.0D, 0.0D);
		}
		arcLengths[0] = arcLengths[subdivs + 1] = new Vec3(0, 0, 0.000001F);
	...
	private double getArcLength(double start, double end, int subdivs) {
		...
		Vec3 prev = null;
		for(int i = 0; i <= subdivs; i++) {
			Vec3 interp = this.spline.interpolate((float)(start + (i * delta)));
			if(prev != null) {
