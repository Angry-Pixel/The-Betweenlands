...
import static thebetweenlands.client.handler.FogHandler.BIOME_FOGS;
...
public class FogState {
	 ...
	 * @return
	 */
	public Fog getBiomeFog(Level level, Vec3 position, float farPlaneDistance, int mode) {
		...
		BiomeFogBetweenlands biomefog = BIOME_FOGS.get(level.getBiome(new BlockPos((int)position.x, (int)position.y, (int)position.z)).getKey());

		if(biomefog != null) {
			float[] fogColor = biomefog.getFogRGB();
			defaultBiomeFog.setRed(fogColor[0]).setGreen(fogColor[1]).setBlue(fogColor[2]);
			defaultBiomeFog.setStart(biomefog.getFogStart(farPlaneDistance, mode)).setEnd(biomefog.getFogEnd(farPlaneDistance, mode));
		}
	 ...
	 * @return
	 */
	public Fog getAmbientFog(Fog biomeFog, Level level, Vec3 position) {
		...
		if(position.y < TheBetweenlands.CAVE_START) {
			float fogColorMultiplier = ((float)(TheBetweenlands.CAVE_START - position.y) / TheBetweenlands.CAVE_START);
			...
			if(position.y <= TheBetweenlands.PITSTONE_HEIGHT) {
				float targettedMultiplier = 0.3F;
				if(fogColorMultiplier < targettedMultiplier) {
					fogColorMultiplier += Math.pow(((targettedMultiplier - fogColorMultiplier) / TheBetweenlands.PITSTONE_HEIGHT * (TheBetweenlands.PITSTONE_HEIGHT - position.y)), 0.85F);
				}
			}
			fogColorMultiplier = Math.clamp(fogColorMultiplier, 0.1F, 1);
	 ...
	 * @return
	 */
	public Fog getAmbientFog(Level level, Vec3 position, float farPlaneDistance, int mode) {
		return this.getAmbientFog(this.getBiomeFog(level, position, farPlaneDistance, mode), level, position);
	}
	 ...
	 * @param mode
	 */
	public void update(Level level, Vec3 position, float farPlaneDistance, int mode) {
		Fog defaultBiomeFog = this.getBiomeFog(level, position, farPlaneDistance, mode);
		Fog defaultAmbientFog = this.getAmbientFog(defaultBiomeFog, level, position);

		NeoForge.EVENT_BUS.post(new UpdateFogEvent(this, defaultBiomeFog, defaultAmbientFog, position, level, farPlaneDistance));
		...
		for(int i = 0; i < 2; i++) {
			...
			float fogPositionIncr = fogPositionDiff / farPlaneDistance / 2.0f * target.getDistanceIncrementMultiplier() * 2.0f;
		...
		for(int a = 0; a < 3; a++) {
			if(currentFogColor[a] != targetFogColor[a]) {
				if(currentFogColor[a] < targetFogColor[a]) {
					currentFogColor[a] += target.getColorIncrement() * 2.0f;
					if(currentFogColor[a] > targetFogColor[a]) {
		...
		if(currentFogDensity != targetFogDensity) {
			if(currentFogDensity < targetFogDensity) {
				currentFogDensity += target.getDensityIncrement() * 2.0f;
				if(currentFogDensity > targetFogDensity) {
