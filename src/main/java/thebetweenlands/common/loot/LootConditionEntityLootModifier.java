...
public class LootConditionEntityLootModifier implements LootCondition {
	private final float min, max;
	private final String key;
	private final boolean invert;

	public LootConditionEntityLootModifier(String key, float min, float max, boolean invert) {
		this.key = key;
		this.min = min;
		this.max = max;
		this.invert = invert;
	...
	public static class Serializer extends LootCondition.Serializer<LootConditionEntityLootModifier> {
		public Serializer() {
			super(new ResourceLocation(ModInfo.ID, "killed_loot_modifier"), LootConditionEntityLootModifier.class);
		}

		@Override
		public void serialize(JsonObject json, LootConditionEntityLootModifier value, JsonSerializationContext context) {
		...
		@Override
		public LootConditionEntityLootModifier deserialize(JsonObject json, JsonDeserializationContext context) {
			...
			return new LootConditionEntityLootModifier(key, min, max, invert);
		}
	}
}