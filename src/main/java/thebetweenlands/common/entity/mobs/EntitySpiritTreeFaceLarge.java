	...
	public void handleStatusUpdate(byte id) {
		...
		if(id == EVENT_ATTACKED) {
			...
			for(int i = 0; i < 32; i++) {
				...
				Vec3d vec = new Vec3d(rx, ry, rz);
				vec = vec.normalize();
				BLParticles.MOTION_ITEM_BREAKING.spawn(world, frontCenter.x, frontCenter.y - 0.25D, frontCenter.z, ParticleArgs.get().withMotion(vec.x * 0.25F, vec.y * 0.25F, vec.z * 0.25F).withData(new ItemStack(ItemRegistry.SAP_SPIT, 0)));
			}
		}
	}
