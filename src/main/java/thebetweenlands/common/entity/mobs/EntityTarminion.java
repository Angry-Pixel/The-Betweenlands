		...
		if(!this.isDead) {
			if(this.getAttackTarget() != null) {
				if(this.world.isRemote) {
					for(int i = 0; i < 200; i++) {
						...
						vec = vec.normalize();
						BLParticles.SPLASH_TAR.spawn(this.world, this.posX + rx + 0.1F, this.posY + ry, this.posZ + rz + 0.1F, ParticleArgs.get().withMotion(vec.xCoord * 0.4F, vec.yCoord * 0.4F, vec.zCoord * 0.4F));
					}
				} else {
					for(int i = 0; i < 8; i++) {
			...
			if(this.world.isRemote) {
				for(int i = 0; i < 100; i++) {
					...
					Vec3d vec = new Vec3d(rx, ry, rz);
					vec = vec.normalize();
					BLParticles.SPLASH_TAR.spawn(this.world, this.posX + rx + 0.1F, this.posY + ry, this.posZ + rz + 0.1F, ParticleArgs.get().withMotion(vec.xCoord * 0.2F, vec.yCoord * 0.2F, vec.zCoord * 0.2F));
				}
			}
		}
		super.setDead();
