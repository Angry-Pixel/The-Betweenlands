    ...
    static class EntityAIMoveTowardsTargetWithDistance extends EntityAIBase {
		...
		public boolean shouldExecute() {
			...
			if (target == null) {
				...
				Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockTowards((EntityCreature) target, 16, 7, new Vec3d(target.posX, target.posY, target.posZ));
				if (vec3d == null) {
					return false;
				} else {
					flyToLocation(vec3d);
				 ...
				 Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockAwayFrom((EntityCreature) target, 16, 7, new Vec3d(target.posX, target.posY, target.posZ));
				if (vec3d == null) {
					return false;
				} else {
					flyToLocation(vec3d);
