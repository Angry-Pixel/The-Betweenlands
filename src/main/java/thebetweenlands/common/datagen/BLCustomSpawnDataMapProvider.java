...
public class BLCustomSpawnDataMapProvider extends DataMapProvider {
    ...
    public static final DataMapType<EntityType<?>, List<ICustomSpawnEntry>> CUSTOM_SPAWNS = DataMapType
            .builder(
                TheBetweenlands.prefix("custom_spawns"),
                Registries.ENTITY_TYPE,
                MobSpawnDataMap.MASTER_LIST_CODEC
            )
            .synced(MobSpawnDataMap.MASTER_LIST_CODEC, false)
            .build();
	...
	public BLCustomSpawnDataMapProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider) {
	 ...
	 @Override
	    protected void gather() {
	        HolderLookup.Provider registries = this.lookupProvider.resultNow();
	        var biomeLookup = registries.lookupOrThrow(Registries.BIOME);
	        
	        // We as we may need to do this a few different ways, so I'm keeping it open
	        ...
	        //this now has all the variables from the old spawner
	        //TODO Implement world ticker to run spawning
