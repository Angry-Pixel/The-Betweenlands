...
public class BlockSwampWater extends BlockFluidClassic implements IStateMappedBlock, ITintedBlock {
    ...
    public IBlockState getExtendedState(IBlockState oldState, IBlockAccess worldIn, BlockPos pos) {
        ...
        height[1][1] = getFluidHeightForRender(worldIn, pos, worldIn.getBlockState(pos.add(0, 1, 0)));
        if (height[1][1] == 1) {
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    corner[i][j] = 1;
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (i != 1 || j != 1) {
                        height[i][j] = getFluidHeightForRender(worldIn, pos.add(i - 1, 0, j - 1), worldIn.getBlockState(pos.add(i - 1, 1, j - 1)));
                    }
                }
            }
            for (int i = 0; i < 2; i++) {
