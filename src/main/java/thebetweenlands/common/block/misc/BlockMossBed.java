...
public class BlockMossBed extends BlockBed implements IStateMappedBlock, ICustomItemBlock {
	...
	public IItemProvider getItemDropped(IBlockState state, World world, BlockPos pos, int fortune) {
		return state.get(PART) == BlockBed.EnumPartType.HEAD ? Items.AIR : ItemRegistry.MOSS_BED_ITEM;
	}
	...
	public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity te, ItemStack stack) {
		if (state.get(PART) == BlockBed.EnumPartType.HEAD && te instanceof TileEntityMossBed) {
			spawnAsEntity(worldIn, pos, new ItemStack(ItemRegistry.MOSS_BED_ITEM));
		} else {
