...
public abstract class WorldGenGiantTree implements IWorldGenerator {
				...
				for (int dz = -radius; dz <= radius; dz++) {
					...
					case 0:
						if (world.getBlockState(new BlockPos(blockX + dx, y - 1, blockZ + dz)).getBlock() == BlockRegistry.LOG_WEEDWOOD) {
							world.setBlockState(new BlockPos(blockX + dx, y - 1, blockZ + dz), BARK);
						}
						break;
					case 1:
				...
				for (int z = minZ; z <= maxZ; z++) {
					if (world.getBlockState(new BlockPos(x, y, z)) == BARK) {
						...
						if (
							...
							world.setBlockState(new BlockPos(x, y, z), WOOD);
						}
					}
				}
			}
	...
	private boolean isValidBiome(Biome biomeBase) {
		return biomeBase == BiomeRegistry.SWAMPLANDS || biomeBase == BiomeRegistry.COARSE_ISLANDS || biomeBase == BiomeRegistry.PATCHY_ISLANDS;
	}
	...
	protected void placeWood(World world, Random rand, int radius, int height, int blockX, int blockY, int blockZ, int dx, int dy, int dz) {
		...
		if(isDentrothyst) {
			world.setBlockState(new BlockPos(blockX + dx, blockY + dy, blockZ + dz), rand.nextInt(20) == 0 ? BlockRegistry.DENTROTHYST.getDefaultState().withProperty(BlockDentrothyst.TYPE, EnumDentrothyst.ORANGE) : BlockRegistry.DENTROTHYST.getDefaultState(), 2);
		} else {
			world.setBlockState(new BlockPos(blockX + dx, blockY + dy, blockZ + dz), WOOD);
		}
	}
	...
	protected void placeBark(World world, Random rand, int radius, int height, int blockX, int blockY, int blockZ, int dx, int dy, int dz) {
		world.setBlockState(new BlockPos(blockX + dx, blockY + dy, blockZ + dz), BARK, 2);		
	}
