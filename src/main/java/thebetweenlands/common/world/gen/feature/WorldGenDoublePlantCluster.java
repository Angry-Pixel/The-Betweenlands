...
public class WorldGenDoublePlantCluster extends WorldGenerator {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		for (int i = 0; i < this.attempts; ++i) {
			...
			if ((worldIn.isAirBlock(blockpos) || (this.isUnderwater && worldIn.getBlockState(blockpos).getMaterial().isLiquid())) && (!worldIn.provider.getHasNoSky() || blockpos.getY() < 254) && this.block.canPlaceBlockAt(worldIn, blockpos)) {
