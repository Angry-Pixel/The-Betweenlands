...
// converted to a vanilla feature wrapper
// makes creating feature generators just like 1.12
public abstract class WorldGenHelper {
	...
	public boolean isReplaceable(Level world, int x, int y, int z, int offsetX, int offsetY, int offsetZ, int rotation) {
		...
		switch (rotation) {
			case 0:
				...
				return world.isLoaded(pos) && (world.getBlockState(pos).canBeReplaced(Fluids.EMPTY) || (replaceable != null && checkReplaceablePredicates(world.getBlockState(pos))));
			case 1:
	...
	public boolean isReplaceable(WorldGenLevel world, int x, int y, int z, int offsetX, int offsetY, int offsetZ, int rotation) {
		x -= width / 2;
		z -= depth / 2;
		BlockPos pos;
		switch (rotation) {
			case 0:
				pos = this.getCheckPos(x + offsetX, y + offsetY, z + offsetZ);
				return world.ensureCanWrite(pos) && (world.getBlockState(pos).canBeReplaced(Fluids.EMPTY) || (replaceable != null && checkReplaceablePredicates(world.getBlockState(pos))));
			case 1:
				pos = this.getCheckPos(x + offsetZ, y + offsetY, z + depth - offsetX - 1);
				return world.ensureCanWrite(pos) && (world.getBlockState(pos).canBeReplaced(Fluids.EMPTY) || (replaceable != null && checkReplaceablePredicates(world.getBlockState(pos))));
			case 2:
				...
				for (int yy = y + offsetB; yy < y + offsetB + sizeHeight; yy++) {
					for (int xx = x + offsetA; xx < x + offsetA + sizeWidth; xx++) {
						for (int zz = z + offsetC; zz < z + offsetC + sizeDepth; zz++) {
							if (!isReplaceable(world, xx, yy, zz, 0, 0, 0, 0))
								return true;
						}
					}
				}
	...
	public boolean rotatedCubeCantReplace(WorldGenLevel world, int x, int y, int z, int offsetA, int offsetB, int offsetC, int sizeWidth, int sizeHeight, int sizeDepth, int direction) {
		x -= width / 2;
		z -= depth / 2;
		switch (direction) {
			case 0 -> {
				for (int yy = y + offsetB; yy < y + offsetB + sizeHeight; yy++) {
					for (int xx = x + offsetA; xx < x + offsetA + sizeWidth; xx++) {
						for (int zz = z + offsetC; zz < z + offsetC + sizeDepth; zz++) {
							if (!world.ensureCanWrite(this.getCheckPos(xx, yy, zz)) || !isReplaceable(world, xx, yy, zz, 0, 0, 0, 0))
								return true;
						}
					}
				}
