...
public class PortalTree {
			...
			for (int i = radius * -1; i <= radius; ++i)
				for (int j = radius * -1; j <= radius; ++j) {
					...
					if (Math.round(Math.sqrt(dSq)) < radius && yy <= 1)
						level.setBlockAndUpdate(pos.offset(i, yy, j), WOOD);
					if (Math.round(Math.sqrt(dSq)) == radius && yy == 0 || Math.round(Math.sqrt(dSq)) == radius && yy <= height - 1)
						level.setBlockAndUpdate(pos.offset(i, yy, j), BARK);
					if (Math.round(Math.sqrt(dSq)) < radius && yy <= height - 1 && yy > 1 && yy <= 10)
						level.setBlockAndUpdate(pos.offset(i, yy, j), Blocks.AIR.defaultBlockState());
				}

			if(yy == 4) {
				TreePortalBlock.makePortalX(level, pos.offset(radius, yy - 2, 0));
				TreePortalBlock.makePortalX(level, pos.offset(- radius, yy - 2, 0));
				TreePortalBlock.makePortalZ(level, pos.offset(0, yy - 2, radius));
				TreePortalBlock.makePortalZ(level, pos.offset(0, yy - 2, - radius));
			}
	...
	private static void createSmallBranch(Level level, BlockPos pos, int dir, int branchLength) {
		...
		for (int i = 0; i <= branchLength; ++i) {
			...
			switch (dir) {
				case 1 -> level.setBlockAndUpdate(pos.east(i).above(y), BARK);
				case 2 -> level.setBlockAndUpdate(pos.west(i).above(y), BARK);
				case 3 -> level.setBlockAndUpdate(pos.south(i).above(y), BARK);
				case 4 -> level.setBlockAndUpdate(pos.north(i).above(y), BARK);
				case 5 -> level.setBlockAndUpdate(pos.east(i).above(y).south(i), BARK);
				case 6 -> level.setBlockAndUpdate(pos.west(i).above(y).north(i), BARK);
				case 7 -> level.setBlockAndUpdate(pos.west(i).above(y).south(i), BARK);
				case 8 -> level.setBlockAndUpdate(pos.east(i).above(y).north(i), BARK);
			}
		}
	}
	...
	private static void createBranch(Level level, RandomSource rand, BlockPos pos, int dir, boolean root, int branchLength) {
		...
		for (int i = 0; i <= branchLength; ++i) {
			...
			switch (dir) {
				case 1:
					if (!root)
						level.setBlockAndUpdate(pos.east(i).above(y), BARK);
					else {
						level.setBlockAndUpdate(pos.east(i).below(y), BARK);
						level.setBlockAndUpdate(pos.east(i).below(y - 1), BARK);
					}
					break;

				case 2:
					if (!root)
						level.setBlockAndUpdate(pos.west(i).above(y), BARK);
					else {
						level.setBlockAndUpdate(pos.west(i).below(y), BARK);
						level.setBlockAndUpdate(pos.west(i).below(y - 1), BARK);
					}
					break;

				case 3:
					if (!root)
						level.setBlockAndUpdate(pos.south(i).above(y), BARK);
					else {
						level.setBlockAndUpdate(pos.south(i).below(y), BARK);
						level.setBlockAndUpdate(pos.south(i).below(y - 1), BARK);
					}
					break;

				case 4:
					if (!root)
						level.setBlockAndUpdate(pos.north(i).above(y), BARK);
					else {
						level.setBlockAndUpdate(pos.north(i).below(y), BARK);
						level.setBlockAndUpdate(pos.north(i).below(y - 1), BARK);
					}
					break;

				case 5:
					if (!root)
						level.setBlockAndUpdate(pos.east(i - 1).above(y).south(i - 1),
							BARK);
					else {
						level.setBlockAndUpdate(pos.east(i - 1).below(y).south(i - 1), BARK);
						level.setBlockAndUpdate(pos.east(i - 1).below(y - 1).south(i - 1), BARK);
					}
					break;

				case 6:
					if (!root)
						level.setBlockAndUpdate(pos.west(i - 1).above(y).north(i - 1), BARK);
					else {
						level.setBlockAndUpdate(pos.west(i - 1).below(y).north(i - 1), BARK);
						level.setBlockAndUpdate(pos.west(i - 1).below(y - 1).north(i - 1), BARK);
					}
					break;

				case 7:
					if (!root)
						level.setBlockAndUpdate(pos.west(i - 1).above(y).south(i - 1), BARK);
					else {
						level.setBlockAndUpdate(pos.west(i - 1).below(y).south(i - 1), BARK);
						level.setBlockAndUpdate(pos.west(i - 1).below(y - 1).south(i - 1), BARK);
					}
					break;

				case 8:
					if (!root)
						level.setBlockAndUpdate(pos.east(i - 1).above(y).north(i - 1), BARK);
					else {
						level.setBlockAndUpdate(pos.east(i - 1).below(y).north(i - 1), BARK);
						level.setBlockAndUpdate(pos.east(i - 1).below(y - 1).north(i - 1), BARK);
					}
					break;
			}
		}
