...
public class ElixirPetrify extends ElixirEffect {
	...
	protected void performEffect(EntityLivingBase livingBase, int strength) {
		livingBase.setLocationAndAngles(MathHelper.floor(livingBase.posX) + 0.5, livingBase.onGround ? MathHelper.floor(livingBase.getBoundingBox().minY) : livingBase.getBoundingBox().minY, MathHelper.floor(livingBase.posZ) + 0.5, 0, 0);
