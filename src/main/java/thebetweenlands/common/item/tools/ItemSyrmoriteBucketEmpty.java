	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemStack, World world, EntityPlayer player, EnumHand hand) {
		if(!world.isRemote) {
			if(player.inventory.hasItemStack(EnumItemMisc.SWAMP_REED_ROPE.create(1))) {
				...
				player.addChatMessage(new TextComponentTranslation("chat.tap.needsRope", new TextComponentTranslation(BlockRegistry.SYRMORITE_RUBBER_TAP.getUnlocalizedName() + ".name")));
			}
		}
