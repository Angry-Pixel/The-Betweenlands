...
public class ShadowStaffItem extends Item implements BigSwingAnimation {

	public ShadowStaffItem(Properties properties) {
		super(properties);
	}
				...
				for (int distance = -1; distance <= 16; distance++) {
					for (int distance2 = -distance; distance2 <= distance; distance2++) {
						for (int yo = 0; yo <= 1; yo++) {
							...
							if (isShadowableBlock(level, player, origin, block)) {
