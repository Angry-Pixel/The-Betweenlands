		...
		if(ctx.getServerHandler() != null) {
			...
			if(player.hasCapability(CapabilityRegistry.CAPABILITY_SUMMON, null)) {
				...
				if(!this.active && cap.isActive()) {
					...
					player.world.play(null, player.posX, player.posY, player.posZ, SoundRegistry.PEAT_MUMMY_CHARGE, SoundCategory.PLAYERS, 0.4F, (player.world.rand.nextFloat() * 0.4F + 0.8F) * 0.8F);
				}
			}
		}
		return null;
